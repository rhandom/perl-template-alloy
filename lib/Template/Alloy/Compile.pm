...
sub compile_template {
    ...
    local $self->{'_encode'} = ($self->{'ENCODING'} && eval { require Encode })
        ? sub { "Encode::decode(\$enc, '".Encode::encode($self->{'ENCODING'}, $_[0])."')" }
        : sub { "'$_[0]'" };
...
# From file ".($doc->{'_filename'} || $doc->{'name'}) . ($self->{'ENCODING'} ? "
...
sub compile_tree {
    ...
    for my $node (@$tree) {
        ...
        if (! ref $node) {
            $node =~ s/([\'\\])/\\$1/g;
            $code .= "\n\n${indent}\$\$out_ref .= ".$self->{'_encode'}->($node).";";
            next;
        }
