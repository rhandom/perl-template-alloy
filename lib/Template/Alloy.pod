...
Template::Alloy - Fast and lightweight TT2/3 template engine
...
the features of TT2 as well as some from TT3.  Template::Alloy is a
full-featured implementation of the Template::Toolkit language.

As of version 2.13, Template::Alloy also provides near full
...
methods which allow Alloy to provide the HTML::Template interface.  It
was possible to add this extra functionality because Template::Alloy
...
Template::Alloy is fast but Template::Alloy::XS is even faster.
If Template::Alloy isn't fast enough for you, the XS version has key
methods coded in C and provides a noticable improvement over the
non-XS version.  Alloy by itself is generally faster than TT, HT, and HTE.
The XS version is nearly always faster - even than HTJ.  Alloy also uses less
...
between Alloy and TT will explain what too look out for.
...
Alloy uses the same base template syntax and configuration items as TT2,
but the internals of Alloy were written from scratch.  Additionally much
...
that the configuration and syntax of Alloy are different from that of
...
engine and will use the same configuration as the current process.  Alloy is several times
...
engine this will allow for more concise generated code.  Because Alloy does
not generated perl code to be executed later, Alloy provides for these virtual
...
The available choices are:
    ...
    cet - Template::Alloy style - the same as TT3
