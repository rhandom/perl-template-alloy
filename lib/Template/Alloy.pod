...
=head2 JavaScript style usage (requires Template::Alloy::JS)
    ...
    my $str = "[% vars.foo = 1 + 3; write('(' + vars.foo + ') (' + vars.key1 + ')'); %]";
    my $out = '';
...
can load all of the roles (except the JS role) by passing "all" to the use statement.
...
This requires the Template::Alloy::JS module to be installed.

Allow eval'ing the block of text as javascript.  The block will be parsed and then eval'ed.

    [% a = "BimBam" %]
    [%~ JS %]
        write('The variable a was "' + vars.a + '"');
        vars.b = "FooBar";
    [% END %]
    [% b %]
...
Requires installation of Template::Alloy::JS.  When enabled, the parsed
...
however).  These limitations are do to the nature of JavaScript::V8
