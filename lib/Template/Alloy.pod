...
flag to 1.  If you are running in a cached environment such as
mod_perl, this is the fastest option.  If you are running in a
non-cached environment, the Playing or the AST is generally faster.
The AST method is also more secure as cached AST won't ever eval
"perl" (assuming PERL blocks are disabled).
...
the current namespace - it is autoloading the role and methods into the
...
Add more POD and examples for all of the various syntaxes.

=item

Allow dispatch to different syntaxes based on file extension.
...
Include HTML::Parser variants such as TAL and Seamstress.
...
If true, allows calls to PROCESS and INCLUDE to temporarily add
the directory of the current template being processed to the INCLUDE_PATHS
arrayref.  This allows templates to refer to files in the local directory
without specifying the local directory.  Default is 0.  If set to a negative
value, the current directory will be added to the end of the current
INCLUDE_PATHS.  This modification to the INCLUDE_PATHS will only be in
effect during a PROCESS or INCLUDE directive.  This feature is similar to
the default behavior of HTML::Template.
